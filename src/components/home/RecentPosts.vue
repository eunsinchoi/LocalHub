...
import { getPosts } from '../services/postStorageService.js'
...
onMounted(async () => {
  ...
  try {
    ...
    // 2) if not found, search user posts
    if (!post.value) {
      const user = getPosts().find((p) => String(p.id) === String(id))
      if (user) {
        post.value = {
          contentid: user.id,
          title: user.title,
          firstimage: user.raw?.image || '',
          firstimage2: '',
          addr1: '',
          addr2: '',
          tel: '',
          modifiedtime: user.createdAt || user.createdAt === 0 ? user.createdAt : user.createdAt || '',
          categoryName: '사용자 게시글',
      ...
      <article v-else-if="post" class="post-card">
        ...
        <div class="post-body">
          ...
          <dl class="post-info">
            ...
            <div class="info-row" v-if="post.overview">
              <dt>내용</dt>
              <dd>{{ post.overview }}</dd>
            </div>
          </dl>
...
<style scoped>
/* keep same styles as before (omitted for brevity) */
.post-detail-view { padding:20px; max-width:960px; margin:0 auto; }
.state { padding:24px; text-align:center; color:#666; }
.state.error { color:#b00020; }
.post-card { background:#fff; border-radius:10px; padding:20px; box-shadow:0 6px 18px rgba(12,12,12,0.06); margin-bottom:18px; }
.post-header { display:flex; flex-direction:column; gap:8px; margin-bottom:12px; }
.post-title { margin:0; font-size:22px; color:#111; }
.post-meta { display:flex; gap:12px; color:#777; font-size:13px; align-items:center; }
.post-image { margin:12px 0; display:block; }
.post-image img { width:100%; height:auto; border-radius:8px; object-fit:cover; }
.post-info { margin-top:8px; display:grid; gap:8px; }
.info-row { display:flex; gap:12px; }
dt { min-width:54px; color:#666; font-weight:600; }
